...
# Paleta de colores - Tema DHL Profesional
COLORS = {
    'bg_primary': '#f8f9fa',
    'bg_secondary': '#ffcc00',
    'bg_card': '#ffffff',
...
class ShadowFrame(tk.Frame):
    """Frame con efecto de sombra simulada"""
    def __init__(self, parent, **kwargs):
        shadow_color = kwargs.pop('shadow_color', '#dee2e6')
        super().__init__(parent, **kwargs)

        self.shadow = tk.Frame(parent, bg=shadow_color)
        self.shadow.place(in_=self, x=3, y=3, relwidth=1, relheight=1)
        self.lift()

class ModernButton(tk.Canvas):
    def __init__(self, parent, text, command, bg_color, hover_color, width=160, height=50):
        ...
        self.rect = self.create_rectangle(3, 3, width-3, height-3, fill=bg_color,
...
class App(tk.Tk):
    ...
    def create_widgets(self):
        # BANNER SUPERIOR DE ESTADO - MUY VISIBLE
        self.status_banner = tk.Frame(self, height=100, bg=COLORS['inactive'])
        ...
        title_label = tk.Label(header_left, text="CLASIFICADOR DHL SHEPORA",
                              bg=COLORS['bg_secondary'], fg=COLORS['accent'],
                              font=('Segoe UI', 22, 'bold'))
        title_label.pack(anchor='w')

        subtitle_label = tk.Label(header_left, text="Sistema Profesional de Gestión de Paquetería",
        ...
        tk.Label(total_left, text="TOTAL REGISTRADOS",
        ...
        scan_display.pack(fill='x', pady=(0, 15))

        scan_inner = tk.Frame(scan_display, bg=COLORS['bg_card'])
        scan_inner.pack(padx=30, pady=20)
        ...
        control_inner.pack(fill='x', padx=30, pady=15)
    ...
    def set_active_mode(self):
        """Modo visual ACTIVO - muy obvio con animación"""
        self.status_banner.config(bg=COLORS['success'])
        for widget in self.status_banner.winfo_children():
            self.set_bg_recursive(widget, COLORS['success'])
    ...
    def set_bg_recursive(self, widget, color):
        """Cambia el background de un widget y sus hijos recursivamente"""
        try:
    ...
    def start_pulse_animation(self):
        """Animación de pulso en el banner activo"""
        ...
        for widget in self.status_banner.winfo_children():
            self.set_bg_recursive(widget, new_bg)
    ...
    def stop_process(self):
        ...
        for widget in self.status_banner.winfo_children():
            self.set_bg_recursive(widget, COLORS['warning'])
